...
// 5. Update maindoc
$stmt = $conn->prepare("UPDATE maindoc SET receiver_signature = ?, receivedBy = ?, receiver_pod = ?, receiver_pod_filename = ?, receiver_pod_mime_type = ?, status = 'Received', dateReceived = NOW() WHERE transactionID = ?");
if (!$stmt) {
...
if (!$success) {
    echo json_encode(['success' => false, 'errors' => ['Failed to update document: ' . $conn->error]]);
    exit;
}
